...
const mockPetsFull = [
  {
    ...
    ai_description:
      "Rikis yra energingas retriveris, kuris dievina žaidimus lauke.",
  ...
  },
  {
    ...
    ai_description:
      "Elegantiška Siamo katė, vertinanti ramybę ir žmogaus draugiją.",
...
const getAllAvailablePets = async (filters = {}) => {
  ...
  if (filters.weightMin) {
    pets = pets.filter(
      (pet) => Number(pet.weight) >= Number(filters.weightMin),
    );
  }
  if (filters.weightMax) {
    pets = pets.filter(
      (pet) => Number(pet.weight) <= Number(filters.weightMax),
    );
  }
