...
const MainAnimalsPage = () => {
    ...
    useEffect(() => {
        fetchPets();
    }, [species, breed, size, activity, city, ageMin, ageMax, weightMin, weightMax]);
    ...
    const speciesOptions = Array.from(new Set(pets.map(p => p.species))).filter(Boolean).sort();
    const breedOptions = Array.from(new Set(pets.map(p => p.breed))).filter(Boolean).sort();
    const sizeOptions = Array.from(new Set(pets.map(p => p.size))).filter(Boolean).sort();
    const activityOptions = Array.from(new Set(pets.map(p => p.activity_level))).filter(Boolean).sort();
    const cityOptions = Array.from(new Set(pets.map(p => p.city))).filter(Boolean).sort();
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ flex: '0 0 calc(20% - 12.8px)', maxWidth: '200px' }}>
                                <label style={{
                                    display: 'block',
                                    marginBottom: '8px',
                                    color: '#495057',
